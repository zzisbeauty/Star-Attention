...
def get_formatted_input(context, question, examples, instruction, post_prompt, chat_template=None):
    ...
    context_template, query_template = CONTEXT_TEMPLATE, QUERY_TEMPLATE
    if chat_template is not None:
        context_format, query_format = chat_template.split('{task_template}')
        context_template = context_format + context_template
        query_template = query_template + query_format

    formatted_context = context_template.format(
        instruction=instruction, examples=examples, post_prompt=post_prompt, context=context.strip()
    ).strip()
    formatted_query = query_template.format(question=question).strip()
